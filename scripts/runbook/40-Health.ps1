...
# Prefer the current process scope. If unset, discover the latest scratch RunRoot.
$RunRoot = $env:EMBEDDINGSHIFT_ROOT
if ([string]::IsNullOrWhiteSpace($RunRoot)) {
    $scratchRoot = Join-Path $RepoRoot 'results\_scratch'
    if (Test-Path $scratchRoot) {
        # Expected shape: results\_scratch\<Scenario>\<RunRoot>
        $latest = Get-ChildItem -Path $scratchRoot -Directory -ErrorAction SilentlyContinue |
...
$ResultsDomain = $env:EMBEDDINGSHIFT_RESULTS_DOMAIN
if ([string]::IsNullOrWhiteSpace($ResultsDomain)) { $ResultsDomain = 'insurance' }
...
Write-Host "[Health] Domain   = $ResultsDomain"
...
if ([string]::IsNullOrWhiteSpace($DatasetRoot) -and $RunRoot) {
    $datasetsBase = Join-Path $RunRoot "results\\$ResultsDomain\\tenants\\$Tenant\\datasets"
...
if ($RunRoot) {
    $runsRoot = Join-Path $RunRoot "results\\$ResultsDomain\\tenants\\$Tenant\\runs"
