...
namespace EmbeddingShift.Tests
{
    ...
    public class MiniRetrievalExperimentWorkflowTests
    {
        ...
        public async Task Mini_retrieval_experiment_produces_baseline_and_variant_metrics()
        {
            ...
            Assert.True(artifacts.Metrics.TryGetValue(MetricKeys.Eval.Map1Baseline,   out var mapBaseline));
            Assert.True(artifacts.Metrics.TryGetValue(MetricKeys.Eval.Ndcg3Baseline, out var ndcgBaseline));
            Assert.True(artifacts.Metrics.TryGetValue(MetricKeys.Eval.Map1Variant,    out var mapVariant));
            Assert.True(artifacts.Metrics.TryGetValue(MetricKeys.Eval.Ndcg3Variant,  out var ndcgVariant));
            Assert.True(artifacts.Metrics.TryGetValue(MetricKeys.Eval.DeltaMap1,      out var deltaMap));
            Assert.True(artifacts.Metrics.TryGetValue(MetricKeys.Eval.DeltaNdcg3,    out var deltaNdcg));
